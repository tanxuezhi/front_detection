...
def get_date(matlab_datenum):
  date = dt.datetime.fromordinal(matlab_datenum - 366.) + dt.timedelta(hours=int((matlab_datenum-int(matlab_datenum))*24))
  return date
...
def read_center_from_mat_file(in_file):
  ...
  for i in range(data['fulllat'].shape[1]):
    ...
    track_date = [get_date(date) for date in track_fulldate]
    track_hh = [int((date - int(date))*24) for date in track_fulldate]

    lat.extend(track_lat)
    lon.extend(track_lon)
    date.extend(track_date)
    yy.extend(track_yy)
    mm.extend(track_mm)
    dd.extend(track_dd)
    hh.extend(track_hh)
